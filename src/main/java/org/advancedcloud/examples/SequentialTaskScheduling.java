...
public class SequentialTaskScheduling {
  ...
  public static void main(String[] args) {
    ...
    OUTPUT_CSV = "results-sequential-" + String.valueOf(totalLimit) + "-" + String.valueOf(CLOUDLETS) + ".csv";
    new SequentialTaskScheduling(offset, totalLimit);
  }

  public SequentialTaskScheduling(int offset, int totalLimit) {
    ...
    // Sequential scheduling (round robin)
    List<Cloudlet> scheduled = scheduleSequential(cloudlets);
  ...
  private List<CloudletDeadline> loadCloudletsFromCSV(String filename, int offset, int totalLimit) {
    ...
    int maxTasksToLoad = Math.min(CLOUDLETS, totalLimit - offset);
  ...
  private List<Cloudlet> scheduleSequential(List<CloudletDeadline> queue) {
    ...
    for (Cloudlet cl : queue) {
      ...
      double newWait = vmWait.get(selectedVm.getId()) + (cl.getLength() / selectedVm.getMips());
